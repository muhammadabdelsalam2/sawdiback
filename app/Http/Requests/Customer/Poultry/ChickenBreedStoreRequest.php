...
class ChickenBreedStoreRequest extends BasePoultryRequest
{
    public function rules(): array
    {
        return [
            'pen_id' => ['nullable', 'integer', Rule::exists('farm_pens', 'id')->where(fn ($q) => $q->where('tenant_id', $this->tenantId()))],
            'code' => ['required', 'string', 'max:100', Rule::unique('poultry_chicken_breeds', 'code')->where(fn ($q) => $q->where('tenant_id', $this->tenantId()))],
