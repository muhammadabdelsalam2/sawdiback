...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="mb-0">{{ __('hr.titles.create_employee') }}</h3>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST"
                  ...
                  action="{{ route('customer.hr.employees.store', ['locale' => request()->route('locale')]) }}">
                @csrf
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="employment_status" class="form-select" required>
                            @foreach($statuses as $status)
                                <option value="{{ $status }}" @selected(old('employment_status', 'active') === $status)>{{ __('hr.options.' . $status) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="operational_department" class="form-select">
                            ...
                            @foreach($operationalDepartments as $department)
                                <option value="{{ $department }}" @selected(old('operational_department') === $department)>{{ __('hr.options.' . $department) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="job_title_id" class="form-select">
                            ...
                            @foreach($jobTitles as $t)
                                <option value="{{ $t->id }}" @selected(old('job_title_id') == $t->id)>{{ $t->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="is_active" class="form-select">
                            <option value="1" @selected(old('is_active', '1') == '1')>{{ __('hr.options.yes') }}</option>
                            <option value="0" @selected(old('is_active') === '0')>{{ __('hr.options.no') }}</option>
                        </select>
                    </div>
                </div>
