...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="mb-0">{{ __('finance.expenses.edit_title') }}</h3>
    ...
    <div class="card"><div class="card-body">
        <form method="POST" action="{{ route('customer.finance.expenses.update', ['locale' => request()->route('locale'), 'expense' => $expense->id]) }}">
            @csrf @method('PUT')
            @include('dashboard.customer.finance.expenses._form')
            <button class="btn btn-primary">{{ __('finance.common.save_changes') }}</button>
