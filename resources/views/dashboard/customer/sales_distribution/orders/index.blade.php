...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="mb-0">{{ __('sales_dist.orders.title') }}</h3>
        <a class="btn btn-primary" href="{{ route('customer.sales-distribution.orders.create', ['locale' => request()->route('locale')]) }}">{{ __('sales_dist.orders.add') }}</a>
    </div>
            ...
            <div class="col-md-2">
                <select name="status" class="form-select">
                    ...
                    @foreach(['draft', 'confirmed', 'fulfilled', 'cancelled'] as $value)
                        <option value="{{ $value }}" @selected(request('status') === $value)>{{ __("sales_dist.status.order.$value") }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="card"><div class="card-body">
        <table class="table align-middle no-datatable sd-export-table"
            data-export-title="{{ __('sales_dist.export.titles.orders_index') }}"
            data-pdf-orientation="landscape" data-pdf-page-size="A4">
            <thead><tr><th>#</th><th>{{ __('sales_dist.orders.fields.order_no') }}</th><th>{{ __('sales_dist.orders.fields.customer') }}</th><th>{{ __('sales_dist.orders.fields.order_date') }}</th><th>{{ __('sales_dist.orders.fields.status') }}</th><th>{{ __('sales_dist.orders.fields.total') }}</th><th class="text-end no-sort no-export">{{ __('sales_dist.common.actions') }}</th></tr></thead>
            <tbody>
            @forelse($orders as $order)
                <tr>
                    ...
                    <td class="text-end">
                        ...
                        <form class="d-inline" method="POST" action="{{ route('customer.sales-distribution.orders.destroy', ['locale' => request()->route('locale'), 'order' => $order->id]) }}">
                            @csrf @method('DELETE')
                            <button class="btn btn-sm btn-outline-danger" onclick="return confirm('{{ __('sales_dist.orders.confirm_delete') }}')">{{ __('sales_dist.common.delete') }}</button>
                        </form>
                    </td>
                </tr>
            @empty
                <tr><td colspan="7" class="text-center text-muted py-4">{{ __('sales_dist.orders.empty') }}</td></tr>
            @endforelse
            </tbody>
        </table>
        {{ $orders->links() }}
