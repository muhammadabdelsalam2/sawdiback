...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="mb-0">{{ __('sales_dist.invoices.title') }}</h3>
        <a class="btn btn-primary" href="{{ route('customer.sales-distribution.invoices.create', ['locale' => request()->route('locale')]) }}">{{ __('sales_dist.invoices.add') }}</a>
    </div>
    ...
    <form class="card card-body mb-3" method="GET">
        <div class="row g-2">
            <div class="col-md-3">
                <select name="status" class="form-select">
                    ...
                    @foreach(['unpaid', 'partially_paid', 'paid', 'void'] as $value)
                        <option value="{{ $value }}" @selected(request('status') === $value)>{{ __("sales_dist.status.invoice.$value") }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-3 d-flex gap-2">
                ...
                <a class="btn btn-light w-100" href="{{ route('customer.sales-distribution.invoices.index', ['locale' => request()->route('locale')]) }}">{{ __('sales_dist.common.reset') }}</a>
            </div>
        </div>
    </form>
            ...
            @forelse($invoices as $invoice)
                <tr>
                    ...
                    <td class="text-end">
                        ...
                        <a class="btn btn-sm btn-outline-secondary" href="{{ route('customer.sales-distribution.invoices.edit', ['locale' => request()->route('locale'), 'invoice' => $invoice->id]) }}">{{ __('sales_dist.common.edit') }}</a>
                        <form class="d-inline" method="POST" action="{{ route('customer.sales-distribution.invoices.destroy', ['locale' => request()->route('locale'), 'invoice' => $invoice->id]) }}">
                            @csrf @method('DELETE')
                            <button class="btn btn-sm btn-outline-danger" onclick="return confirm('{{ __('sales_dist.invoices.confirm_delete') }}')">{{ __('sales_dist.common.delete') }}</button>
                        </form>
                    </td>
                </tr>
            @empty
