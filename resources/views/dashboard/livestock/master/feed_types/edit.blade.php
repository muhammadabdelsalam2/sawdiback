...
@section('content')
    <div class="container py-4 livestock-page">
        ...
        <div class="card-block">
            <form method="POST" action="{{ route('customer.livestock.feed-types.update', ['locale' => $currentLocale, 'feed_type' => $feedType->id]) }}">
                @method('PUT')
                ...
                <div class="mt-3 d-flex gap-2">
                    ...
                    <a class="btn btn-outline-white" href="{{ route('customer.livestock.feed-types.index', ['locale' => $currentLocale]) }}">{{ __('livestock.actions.back') }}</a>
                </div>
            </form>
        </div>
    </div>
