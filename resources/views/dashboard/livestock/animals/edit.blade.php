    ...
    <div class="page-head mb-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <h2 class="page-title mb-1 font-weight-bold text-dark">
                {{ __('livestock.titles.edit_animal') }} 
                <span class="tag-badge">{{ $animal->tag_number }}</span>
            </h2>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent p-0 mb-0">
                    ...
                    <li class="breadcrumb-item">
                        <a href="{{ route('customer.livestock.animals.show', ['locale' => $currentLocale, 'animal' => $animal->id]) }}">{{ $animal->tag_number }}</a>
                    </li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __('livestock.actions.edit') }}</li>
                </ol>
            </nav>
        </div>
    </div>
